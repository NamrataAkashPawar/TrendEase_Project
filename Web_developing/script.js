...
function renderCart() {
  ...
  if (cart.length === 0) {
    list.innerHTML = '<li class="list-group-item text-muted text-center">Your cart is empty.</li>';
  }
  ...
  cart.forEach(product => {
    ...
    li.innerHTML = `${product.name} (x${product.quantity})
...
function openBuyNowModal(id) {
  ...
  modalBody.innerHTML = `<img src="${product.img}" class="img-fluid mb-3" alt="${product.name}">
    <h5>${product.name}</h5>
